...
class The7_FSE_Font_Manager {
	...
	public function fetch_existing_fonts() {
		...
		$font_files = glob( $font_dir . '/*.{woff,woff2,ttf,otf}', GLOB_BRACE );
	...
	public function print_font_faces( $fonts = [] ) {
		...
		$font_dir = untrailingslashit( wp_get_font_dir()['baseurl'] );

		$this->replace_src( $fonts, $font_dir );
	 ...
	 *
	 * @param array  $array   Array to replace the URLs.
	 * @param string $font_dir Font directory.
	 *
	 * @return void
	 */
	public function replace_src( &$array, $font_dir ) {
		foreach ( $array as &$value ) {
			if ( is_array( $value ) ) {
				if ( isset( $value['src'] ) && is_array( $value['src'] ) ) {
					foreach ( $value['src'] as &$src_val ) {
						if ( is_string( $src_val ) && strpos( $src_val, 'the7.io' ) !== false ) {
							$src_val = $font_dir . '/' . basename( $src_val );
						}
					}
				}
				$this->replace_src( $value, $font_dir );
			}
		}
	}
